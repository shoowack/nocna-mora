...
export async function generateMetadata({ params }: Props) {
  ...
  return { title: `${result.docs[0].title} | TV Arhiv` }
}
...
export default async function CategoryDetailPage({ params, searchParams }: Props) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8">
      <h1 className="mb-2 text-3xl font-bold text-foreground">{category.title}</h1>
      ...
      {videos.totalPages > 1 && (
        <div className="mt-8 flex justify-center gap-4">
          {page > 1 && (
            <a href={`/kategorije/${slug}?page=${page - 1}`} className="rounded-md border border-border px-4 py-2 text-sm text-muted-foreground hover:text-foreground">
              Prethodna
            </a>
          )}
